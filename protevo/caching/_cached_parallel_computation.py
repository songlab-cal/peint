...
def secure_parallel_output(output_dir: str, parallel_arg: str, file_type: str = '.txt') -> None:    
    return_code = os.system(f"chmod 444 {os.path.join(output_dir, parallel_arg + file_type)}")
    if return_code == 0:
    ...
    def cached_parallel_computation_decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if len(new_parallel_args):
                ...
                logger.debug(
                    f"Calling {func.__name__} for {len(new_parallel_args)} new parallel args. Output location: {func_caching_dir}"
                )
                if get_read_only():
                    raise CacheUsageError(
