...
**Environment.** Install peint into the `peint`/`protevo` conda env: `pip install -e .`.
...
```bash
git submodule update --init protevo/vep/ProteinGym3
```
...
**2. Build training transitions.** `python -m protevo.vep._dms_datasets` (no args; the
production set is hardcoded to `hhfilter90`, dropping families with <10 sequences).

**3. Train** (needs `.[train]` + a GPU):

```bash
python -m protevo.vep.train_peint_vep \
...
```bash
python -m protevo.vep.compute_fitness \
...
(`python -m protevo.vep.compute_wag_scores` produces the WAG baseline).
